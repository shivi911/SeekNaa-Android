package com.example.shivi.viewpagernews.models;
...
public class News {
    ...
    public static News makeNews(JSONObject newsItemJson) {
        ...
        try {
            ...
            news.title = title;
            news.byline = byline;
            news.newsUrl = newsItemJson.getString("short_url");
            try {
                JSONArray mediaJson = newsItemJson.getJSONArray("multimedia");
                if (mediaJson != null) {

                    // JSONArray mediaMetaJson = mediaJson.getJSONObject(0).getJSONArray("media-metadata");
                   // if (mediaMetaJson != null) {
                        String imageUrl = mediaJson.getJSONObject(0).getString("url");
